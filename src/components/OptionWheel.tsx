...
interface OptionWheelProps {
  ...
  /** Externally driven selection (e.g. scroll-spy). Glides the wheel to the
   *  option without firing onChange, so the parent never scroll-fights the
...
/**
 * OptionWheel (react-bits) - a curved, draggable wheel of options.
 * Options sit on a circle and ease toward the middle as the wheel settles.
 * Keyboard: ArrowUp/ArrowDown (or Left/Right). Wheel/touchpad scroll works too.
 */
const OptionWheel = ({
  items = DEFAULT_ITEMS,
  defaultSelected = 3,
  ...
  // Honor prefers-reduced-motion: snap instantly instead of easing.
  const reduceMotion =
    typeof window !== "undefined" &&
  ...
  cfgRef.current = {
    ...
    side,
    loop,
    smoothing: reduceMotion ? 0 : smoothing,
    draggable,
    soundUrl,
    soundVolume,
  };
  ...
  const playTick = useCallback(() => {
    const cfg = cfgRef.current as { soundUrl: string; soundVolume: number };
  ...
  // External control: when `selected` changes (e.g. the parent's scroll-spy
  // noticed a new section in view), glide the wheel there silently.
  useEffect(() => {
    if (selected == null) return;
    const cfg = cfgRef.current as { count: number };
    const idx = ((Math.round(selected) % cfg.count) + cfg.count) % cfg.count;
    if (idx !== selectedRef.current) applyTarget(idx, true, false);
